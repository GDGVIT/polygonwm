...
static void setshape(Client *c, int ion);
...
setup(void)
{
	...
	XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
		PropModeReplace, (unsigned char *) "polygonwm", 3);
...
spawn(const Arg *arg)
{
	if (fork() == 0) {
		...
		fprintf(stderr, "polygonwm: execvp %s", ((char **)arg->v)[0]);
		perror(" failed");
		exit(EXIT_SUCCESS);
	}
}
...
updatebars(void)
{
	...
	XClassHint ch = {"polygonwm", "polygonwm"};
	for (m = mons; m; m = m->next) {
		if (m->barwin)
			continue;
...
updatestatus(void)
{
	if (!gettextprop(root, XA_WM_NAME, stext, sizeof(stext)))
		strcpy(stext, "polygonwm-"VERSION);
	drawbar(selmon);
}
...
xerror(Display *dpy, XErrorEvent *ee)
{
	...
	|| (ee->request_code == X_CopyArea && ee->error_code == BadDrawable))
		return 0;
	fprintf(stderr, "polygonwm: fatal error: request code=%d, error code=%d\n",
...
{
	die("polygonwm: another window manager is already running");
	return -1;
}
...
//	if (((strcmp(selmon->lt[selmon->sellt]->symbol,"o o")) != 0) && (ion == 0)) {
	if ((ion == 0)) {
...
main(int argc, char *argv[])
{
	if (argc == 2 && !strcmp("-v", argv[1]))
		die("polygonwm-"VERSION);
	else if (argc != 1)
		die("usage: polygonwm [-v]");
	if (!setlocale(LC_CTYPE, "") || !XSupportsLocale())
		fputs("warning: no locale support\n", stderr);
	if (!(dpy = XOpenDisplay(NULL)))
		die("polygonwm: cannot open display");
	checkotherwm();
	setup();
#ifdef __OpenBSD__
